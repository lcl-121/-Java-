...
const save = () => {
  if (form.value.tagIds) {
    form.value.tagIds = form.value.tagIds.join(',');
  } else {
    form.value.tagIds = '';
  }

  request.post("/unit", form.value).then(res => {
    if (res.code === '200') {
...
const handleEdit = (row) => {
  ...
  if (form.value.tagIds) {
    form.value.tagIds = form.value.tagIds.split(',').map(id => Number(id));
  } else {
    form.value.tagIds = [];
  }
...
const delBatch = () => {
  ...
  request.post("/unit/del/batch", ids).then(res => {
    if (res.code === '200') {
...
const getItemNames = (idsString) => {
  if (!idsString) return '';
  const idArray = idsString.split(',');
  const array = idArray.map(id => {
    const item = tags.value.find(item => item.id === id || item.id === Number(id));
  ...
  <div class="content-container">
    ...
    <el-card>
      <el-table :data="tableData" @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="info" label="商家说明"/>
        <el-table-column label="商家标签" min-width="180" align="center">
          <template #default="scope">
            <span v-if="scope.row.tagIds">{{getItemNames(scope.row.tagIds)}}</span>
          </template>
        </el-table-column>
    ...
    <el-dialog v-model="dialogFormVisible" :title="form.id ? '编辑' : '新增'" width="30%" destroy-on-close center>
      <el-form :model="form" label-width="100px">
        ...
        <el-form-item label="商家说明">
          <el-input v-model="form.info" placeholder="请输入商家说明" />
        </el-form-item>
        <el-form-item label="多选选项">
          <el-select v-model="form.tagIds" placeholder="请选择多个标签" style="width:100%" multiple>
            <el-option v-for="item in tags" :key="item.id" :label="item.name" :value="Number(item.id)" />
          </el-select>
        </el-form-item>
      </el-form>
      <template #footer>
