...
const changeState = (row) => {
  form.value = JSON.parse(JSON.stringify(row))
  request.post("/", form.value).then(res => {
    if (res.code === '200') {
      ElMessage.success("保存成功")
      dialogFormVisible.value = false
      load()
  ...
  <div class="content-container">
    ...
    <el-card>
      <el-table :data="tableData" @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="info" label="轮播图说明"/>
        <el-table-column label="分类封面图" width="120" align="center">
        ...
        <el-table-column label="关联商品" width="120" align="center">
          <template #default="scope">
            <span v-if="scope.row.goodsId">{{goods.find(item=>item.id===scope.row.goodsId).name}}</span>
          </template>
        </el-table-column>
    ...
    <el-dialog v-model="dialogFormVisible" :title="form.id ? '编辑' : '新增'" width="30%" destroy-on-close center>
      <el-form :model="form" label-width="100px">
        ...
        <el-form-item label="轮播图标题" required>
          <el-input v-model="form.name" placeholder="轮播图标题" />
        </el-form-item>

        <el-form-item label="轮播图说明" required>
          <el-input v-model="form.name" placeholder="轮播图说明" />
        </el-form-item>
        ...
        <el-form-item label="关联商品" required>
          <el-select v-model="form.goodsId" placeholder="关联商品" style="width: 240px">
            <el-option
              v-for="item in goods"
              :key="item.id"
              :label="item.name"
